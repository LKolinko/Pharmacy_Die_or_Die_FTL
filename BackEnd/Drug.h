...
public:
    ...
    Drug(Json::Value json) {
        ...
        quantity_ = json["quantity"].asString();
        retail_price_ = json["retailPrice"].asString();
    }

    Drug(std::string name, std::string group, std::string type, 
    std::string dosage, std::string expiration_date, std::string quantity, std::string retail_price)
    ...
    bsoncxx::document::value ToBson() {
        auto doc_value = make_document(
            ...
            kvp("quantity", std::stoi(quantity_))
        );
        return doc_value;
    }
    ...
    std::string dosage_, quantity_, expiration_date_;
